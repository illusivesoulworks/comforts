...
public class ComfortsFabricMod implements ModInitializer {
  ...
  public void onInitialize() {
    ComfortsConfigInitializer.init();
    ...
    for (RegistryObject<Block> value : comfortsBlocks) {
      ...
      if (block instanceof BaseComfortsBlock baseComfortsBlock) {
        LandPathTypeRegistry.registerDynamic(block,
                                            (state, blockGetter, pos, neighbor) -> baseComfortsBlock.getBlockPathType(
                                                state, blockGetter, pos, null));
      }
    }
